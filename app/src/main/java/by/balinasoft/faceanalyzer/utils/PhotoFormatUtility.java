...
/*    public static JsonObject prepareJsonImageUid(String photo, String FLAG) {
    ...
    public static JSONObject toXml(String photo, String FLAG) {
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put(Constants.API_KEY, Constants.API);
            jsonObject.put(Constants.API_SECRET, Constants.SECRET);
            jsonObject.put(Constants.DETECTION_FLAGS, FLAG);
            jsonObject.put(Constants.IMAGE_BASE64, photo);
            jsonObject.put(Constants.ORIGINAL_FILENAME, NAME);
        } catch (JSONException e) {
            throw new IllegalArgumentException(String.format("Invalid photo format %s", photo), e);
        }
        return jsonObject;
    }
    ...
    public static String parses(JSONObject jsonObject) {
        try {
            return  jsonObject.getString("img_uid");
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return null;
